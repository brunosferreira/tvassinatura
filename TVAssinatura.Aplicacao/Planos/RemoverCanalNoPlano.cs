...
{
    public class RemoverCanalNoPlano
    {
        private IPlanoRepositorio _planoRepositorio;

        public RemoverCanalNoPlano(IPlanoRepositorio planoRepositorio)
        {
            _planoRepositorio = planoRepositorio;
        }

        public void Remover(int idDoPlano, Canal canal)
        {
            if (!_planoRepositorio.ExisteCanalNoPlano(idDoPlano, canal.Id))
                throw new ArgumentException("Este canal não pertence a este plano");
